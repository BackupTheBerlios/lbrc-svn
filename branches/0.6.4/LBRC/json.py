...
##
##    Matthias Bläsing <matthias.blaesing@rwth-aachen.de> 30-12-2007
##    * Add a pretty_print option, that indents object and array notation
##    * Add lines method to StringGenerator, that reports the count of
##      lines, that were read until that point (used to provide a better
##      method to detect problems in json files)
##    * Handle empty objects, which only contain whitespace
##    * Convert all tabs to spaces (mostly spaces were used to indent)
...
class _StringGenerator(object):
    ...
    def all(self):
        return self.string
    def lines(self):
        return self.string[0:self.index].count("\n")
...
class JsonReader(object):
    ...
    def _read(self):
        ...
        if peek == '{':
            ...
            raise ReadException, "Input is not valid JSON: '%s'\nPeak was: \"%s\"\nLines seen: %i" % (self._generator.all(), peek, self._generator.lines())
...
class JsonWriter(object):
    ...
    def write(self, obj, escaped_forward_slash=False, pretty_print=False):
        self._escaped_forward_slash = escaped_forward_slash
        self._results = []
        if pretty_print:
            self._indent_stack = []
        self._write(obj, pretty_print=pretty_print)
        return "".join(self._results)

    def _write(self, obj, pretty_print=False):
        ty = type(obj)
        if ty is types.DictType:
            n = len(obj)
            self._append("{")
            if pretty_print: self._append("\n"  + (sum(self._indent_stack)+2) * " ")
            if pretty_print: self._indent_stack.append(2)
            first = True
            for k, v in obj.items():
                if pretty_print:
                    if not first: 
                        self._append(sum(self._indent_stack) * " ")
                    else:
                        first = False
                self._write(k, pretty_print)
                self._append(":")
                if pretty_print: self._append(" ")
                self._write(v, pretty_print)
                n = n - 1
                if n > 0:
                    self._append(",")
                    if pretty_print: self._append("\n")
...
def write(obj, escaped_forward_slash=False, pretty_print=False):
    return JsonWriter().write(obj, escaped_forward_slash, pretty_print)
