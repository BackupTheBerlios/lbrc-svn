...
class XInputEditor(ConfigWindowWidget):
    """
    Widget for configuration of UinputDispatcher
    ...
    def __init__(self, config):
        ...
        comborenderer.connect("edited", self._treeview_changed, "type")
        mylist = gtk.ListStore(str)
        for i in x_types_values.values():
            mylist.append([i[4]])
        comborenderer.set_property("model", mylist)
        comborenderer.set_property("text-column", 0)
        comborenderer.set_property("editable", True)
        ...
        comborenderer = gtk.CellRendererText()
        comborenderer.connect("edited", self._treeview_changed, "event")
        comborenderer.set_property("editable", True)      
        treeview.insert_column_with_attributes(
            2, 
    ...
    def update_view(self):
        # 0 => keycode, 1 => map_to, 2 => repeat_freq, 3 => EVENT TYPE, 
        # 4 => BACKGOROUND COLOR, 5 => EVENT TYPE DESCRIPTION 6 => is a repeater
        # 7 => model for the event mapping, 
        # 8 => do we allow other event mappings (useful for keyboard)
        
        mylist = gtk.ListStore(int, str, int, str, str, str, 
        ...
        try:
            for map in self.section['actions']:
                ...
                mylist.append([
                    ...
                    type,
                    "snow",
                    x_types_values[type][4],
                    x_types_values[type][3],
                    x_types_values[type][2],
                    x_types_values[type][0],
                    map
                ])
        except KeyError: pass
        except TypeError: pass
    ...
    def _treeview_changed(self, cellrenderer, treepath, new_text, ctype):
        ...
        if ctype == 'type':
            row["type"] = type = reverse_types[new_text]
            row["map_to"] = map_to = x_types_values[type][1][0]
            repeat_freq = 0
            if repeat_freq in row: del row["repeat_freq"]
        elif ctype == 'keycode':
            row["keycode"] = keycode = int(new_text)
        ...
        model.set(iter, 0, keycode, 1, map_to, 2, repeat_freq, 3, type, 
                        5, x_types_values[type][4], 6, x_types_values[type][3],
                        7, x_types_values[type][2], 8, x_types_values[type][0])
...
class DBusCallerEditor(ConfigWindowWidget):
    ...
    def _treeview_changed(self, cellrenderer, treepath, *args):
        ...
        if ctype == 'keycode':
            ...
            if len(object) == 0:
                del model.get_value(iter, 7)['arguments']
        self.emit('changed')
...
class ConfigWindow(gobject.GObject):
    ...
    def __init__(self):
        ...
        # TODO: Config for Presentation Companion
        # TODO: Config for VolumeControl
        for i in (XInputEditor,
                  DBusCallerEditor, 
                  MPlayerEditor, 
                  CommandExecutorEditor,
                  UinputKeyMouseEditor):
